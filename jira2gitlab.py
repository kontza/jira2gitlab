...
def jira_text_2_gitlab_markdown(jira_project, text, adict):
    ...
    t = re.sub(
        r"\{code:(\w+)(?:\|\w+=[\w.\-]+)*\}\s*", r"\n```\1\n", t
    )  # Block code (with language and properties)
    t = re.sub(
        r"\{code:[^}]*\}\s*", r"\n```\n", t
    )  # Block code (catch-all, bailout to simple)
    t = re.sub(r"\n\s*bq\. (.*)\n", r"\n> \1\n", t)  # Block quote
    t = re.sub(r"\{quote\}", r"\n>>>\n", t)  # Block quote #2
    ...
    t = re.sub(
        r"(\b%s-\d+\b)" % jira_project, r"[\1](%s/browse/\1)" % JIRA_URL, t
...
def move_attachments(attachments, gitlab_project_id):
    ...
    for attachment in attachments:
        ...
        if not _file:
            log.warning(f"Unable to migrate attachment: {attachment['content']} ... ")
            continue

        _content = BytesIO(_file.content)

        file_data = (
            (clean_filename, _content)
            if KEEP_ORIGINAL_ATTACHMENT_FILENAMES
            else (str(uuid.uuid4()), _content)
        )  # Use a UUID as file name

        file_info = requests.post(
            f"{GITLAB_API}/projects/{gitlab_project_id}/uploads",
            headers={
                "PRIVATE-TOKEN": GITLAB_TOKEN,
                "Sudo": resolve_login(author)["username"],
            },
...
def gitlab_user_admin(user, admin):
    ...
    try:
        ...
        raise Exception(
            f"Unable change admin status of Gilab user {user['username']} to {admin}\n{e}"
        )
...
def migrate_user(jira_username):
    log.info(f"Migrating user {jira_username}")
...
def create_gl_project(gitlab_project):
    log.info(f"Creating Gitlab project {gitlab_project}")
...
def migrate_project(jira_project, gitlab_project):
    ...
    try:
        ...
        raise Exception(
            f"Unable to list Gitlab milestones for project {gitlab_project}!\n{e}"
        )
    ...
    for index, issue in enumerate(jira_issues, start=1):
        ...
        if issue["key"] in import_status["issue_mapping"]:
            if import_status["issue_mapping"][issue["key"]][1] == issue_hash:
                log.info(
                    f"Issue {issue['key']} found in status with the same hash: previously imported and not changed."
                )
                continue
            else:
                log.info(
                    f"#{index}/{len(jira_issues)} Jira issue {issue['key']} was imported before, but it has changed. Deleting and re-importing."
                )
                requests.delete(
            ...
            log.info(
                f"#{index}/{len(jira_issues)} Migrating Jira issue {issue['key']} ...   "
            )
        ...
        if issue["fields"]["issuetype"]["name"] in ISSUE_TYPE_MAP:
            ...
            log.warning(
                f"Jira issue type {issue['fields']['issuetype']['name']} not mapped. Importing as generic label."
            )
        ...
        if "priority" in issue["fields"]:
            if (
                ...
                and issue["fields"]["priority"]["name"] in ISSUE_PRIORITY_MAP
            ):
                gl_labels.append(
                    ISSUE_PRIORITY_MAP[issue["fields"]["priority"]["name"]]
                )
            else:
                gl_labels.append(
                    PREFIX_PRIORITY + issue["fields"]["priority"]["name"].lower()
                )
        ...
        if (
            issue["fields"]["status"]
            and issue["fields"]["status"]["name"] in ISSUE_STATUS_MAP
        ):
        ...
        if (
            ...
            and issue["fields"]["resolution"]["name"] in ISSUE_RESOLUTION_MAP
        ):
            gl_labels.append(
                ISSUE_RESOLUTION_MAP[issue["fields"]["resolution"]["name"]]
            )

        # storypoints / weight
        if (
            JIRA_STORY_POINTS_FIELD in issue["fields"]
            and issue["fields"][JIRA_STORY_POINTS_FIELD]
        ):
        ...
        for fixVersion in issue["fields"]["fixVersions"]:
            gl_milestone_id = get_milestone_id(
                gl_milestones, gitlab_project_id, fixVersion["name"]
            )
        ...
        for link in issue["fields"]["issuelinks"]:
            if "outwardIssue" in link:
                import_status["links_todo"].add(
                    (issue["key"], link["type"]["outward"], link["outwardIssue"]["key"])
                )
        ...
        if MIGRATE_ATTACHMENTS and "attachment" in issue["fields"]:
            replacements = move_attachments(
                issue["fields"]["attachment"], gitlab_project_id
            )

        # Create Gitlab issue
        # Add a link to the Jira issue and mention all attachments in the description
        gl_description = jira_text_2_gitlab_markdown(
            jira_project, issue["fields"]["description"], replacements
        )
        gl_description += "\n\n___\n\n"
        ...
        if gl_reporter == GITLAB_ADMIN and reporter != "jira":
            gl_description += (
                f"**Original creator of the issue: Jira user {reporter}**\n\n"
            )
        ...
        try:
            ...
            for comment in issue["fields"]["comment"]["comments"]:
                ...
                note_add = requests.post(
                    ...
                    json={
                        ...
                        "body": notice
                        + jira_text_2_gitlab_markdown(
                            jira_project, comment["body"], replacements
                        ),
                    },
                )
            ...
            if MIGRATE_WORLOGS:
                for worklog in issue["fields"]["worklog"]["worklogs"]:
                    ...
                    if "comment" in worklog:
                        worklog_comment = jira_text_2_gitlab_markdown(
                            jira_project, worklog["comment"], replacements
                        )
            ...
            if (
                ...
                or issue["fields"]["status"]["name"] in ISSUE_STATUS_CLOSED
            ):
            ...
            raise Exception(
                f"Unable to modify Gitlab issue {gl_issue['id']}. Removing issue and aborting.\n{e}"
            )
...
def process_links():
    for j_from, j_type, j_to in import_status["links_todo"].copy():
        log.info(f"Processing link {j_from} {j_type} {j_to}")

        if not (
            j_from in import_status["issue_mapping"]
            and j_to in import_status["issue_mapping"]
        ):
            log.warning(
        ...
        if j_type in ["relates to", "blocks", "causes"]:
            ...
            try:
                ...
                log.error(
                    f"Unable to create Gitlab issue link: {gl_from} {gl_type} {gl_to}\n{e}"
                )
            ...
            if j_type == "duplicates":
                try:
                    note_add = requests.post(
                        ...
                        headers={"PRIVATE-TOKEN": GITLAB_TOKEN},
                        verify=VERIFY_SSL_CERTIFICATE,
                        json={"body": f"/duplicate {gl_to['full_ref']}"},
                    )
                    note_add.raise_for_status()
                except requests.exceptions.RequestException as e:
                    log.error(
                ...
                log.warning(f"Don't know what to do with link type {j_type}!")
...
def load_import_status():
    try:
        ...
        log.info("Creating new import_status file")
        import_status = {
            "issue_mapping": dict(),
...
def reset_user_privileges():
    log.info("Resetting user privileges ...")
...
def final_report():
    if jira_users_not_mapped:
        ...
        log.info(
            f"{json.dumps(jira_users_not_mapped, default=json_encoder, indent=4)}\n"
        )

    if gl_users_not_migrated:
        log.info(
            f"The following Jira users could not be found in Gitlab and could not be migrated. They have been impersonated by {GITLAB_ADMIN} (number of times)"
        )
        log.info(
            f"{json.dumps(gl_users_not_migrated, default=json_encoder, indent=4)}\n"
        )

    if import_status["gl_users_made_admin"]:
        log.info("An error occurred while reverting the admin status of Gitlab users.")
        log.info(
            "IMPORTANT: The following users should be revoked the admin status manually:"
        )
        log.info(
...
def wrapup():
    ...
    try:
        ...
        log.error(f"Could not reset privileges: {e}\n")
...
def sigint_handler(signum, frame):
    log.info("Migration interrupted (SIGINT)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for jira_project, gitlab_project in PROJECTS.items():
            log.info(f"Migrating {jira_project} to {gitlab_project}")
